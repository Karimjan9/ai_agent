...
class RecoverLabEvaluationErrors extends Command
{
    ...
    public function handle(): int
    {
        ...
        if (collect([$afterAuthRepair, $afterServiceRepair, $afterRuntimeSchemaRepair, $afterIpcRepair, $afterRetryBudgetRepair])->filter()->count() > 1) {
        ...
        $agents = LabAgent::query()->with(['modelVersion', 'generation'])
            ...
            ->limit($limit * 3)
            ->get()
            ->filter(function (LabAgent $agent) use ($afterAuthRepair, $afterServiceRepair, $afterRuntimeSchemaRepair, $afterIpcRepair, $afterRetryBudgetRepair, $mode): bool {
                $attempts = (int) data_get($agent->modelVersion?->metadata, 'evaluator_recovery_attempts', 0);
                if ($this->hasQueuedJob($agent, $mode)) return false;
                if ($afterAuthRepair) {
        ...
        DB::transaction(function () use ($agents, $afterAuthRepair, $afterServiceRepair, $afterRuntimeSchemaRepair, $afterIpcRepair, $afterRetryBudgetRepair, $fullRecovery): void {
