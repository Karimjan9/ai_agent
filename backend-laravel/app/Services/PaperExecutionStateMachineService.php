...
{
    private const NEXT = ['locate' => ['trigger', 'abort'], 'trigger' => ['confirm', 'abort'], 'confirm' => ['open', 'abort'], 'open' => ['manage', 'abort', 'review'], 'manage' => ['manage', 'abort', 'review'], 'abort' => ['review'], 'review' => []];
    ...
    public function transition(ModelMarketPerformance $candidate, string $stage, ?PaperSignal $signal = null, ?PaperOrder $order = null, ?string $previous = null, array $telemetry = []): array
    {
        $valid = $previous === null || in_array($stage, self::NEXT[$previous] ?? [], true);
        $event = $this->record($candidate, 'tactic_'.$stage, $signal, $order, ['provider' => 'tactic_state_machine', 'reason' => $valid ? null : 'INVALID_STAGE_TRANSITION', 'payload' => ['previous_stage' => $previous, 'stage' => $stage, 'valid' => $valid, 'telemetry' => $telemetry]]);

        return ['valid' => $valid, 'stage' => $stage, 'event_id' => $event->id, 'promotion_evidence' => false];
    }
}
