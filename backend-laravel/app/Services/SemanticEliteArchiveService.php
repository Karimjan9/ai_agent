 ...
 * bounded set of same-cell capability/novelty contributors. A cell never
 * borrows genetic material from another cell.
 */
class SemanticEliteArchiveService
{
    ...
    /**
     * Keep a bounded frontier inside each exact cell. The historical
     ...
     */
    public function frontierPerCell(iterable $parents, int $limit = 5, ?callable $score = null): Collection
    {
        $score ??= static fn (ModelVersion $model): float => (float) ($model->best_score ?? 0);
        $limit = max(1, $limit);
    ...
    public function contract(ModelVersion $model, ?string $family = null): array
    {
        ...
        return [
            ...
            'cell_key' => $cell,
            'archive_role' => 'local_champion_plus_bounded_same_cell_frontier',
            'genetic_parent_rule' => 'exact_cell_only; bounded_frontier; root_default_when_empty',
