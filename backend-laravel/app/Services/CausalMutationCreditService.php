    ...
    public function reconcileGeneration(int $generationId): int
    {
        ...
        foreach ($agents as $agent) {
            ...
            foreach ($agent->mutationMemories as $memory) {
                ...
                $parent = $parentId ? ModelMarketPerformance::query()->where('model_version_id', $parentId)->first() : null;
                ...
                $independentTemporalLanes = min(
                    (int) $current->rolling_windows_count,
                    (int) $parent->rolling_windows_count,
                    (int) $alternative->rolling_windows_count,
                ) >= 2;
                ...
                $priorConfirmations = MutationMemory::query()
                    ->where('id', '!=', $memory->id)->where('symbol', $memory->symbol)->where('timeframe', $memory->timeframe)
                    ->where('strategy_family', $memory->strategy_family)->where('parameter_key', $memory->parameter_key)
                    ->where('execution_contract_hash', $memory->execution_contract_hash)
                    ->where('non_target_regression_status', 'passed')->where('outcome', 'beneficial')->count();
                $paired = [
                ...
                $memory->update([
                    ...
                    'independent_confirmation_count' => $confirmed ? min(2, $priorConfirmations + 1) : 0,
                    'non_target_regression_status' => data_get($effect, 'differential_no_regression.status', 'passed'),
                    'evidence_scope_status' => $confirmed && $priorConfirmations >= 1 ? 'eligible_prior' : 'historical_failure_memory',
                ]);
